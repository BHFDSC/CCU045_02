...
file.remove(list.files(here::here("figs", "baseline_table", "revision_hosp50"), full.names = TRUE))
...
for (c in seq_along(combined)) {
  export_table(
    combined[[c]], "baseline_table/revision_hosp50", 
    paste0(names(combined)[c], "_revision50")
  )
}
