...
# Analysis data
mi <- read_rds(here::here("output", "multiple_imputation_data_hf_fct.rds"))
...
full <- complete(mi, 'long', include = TRUE) |>
  ...
  dplyr::select(
    ...
    acm_ep, cvd_ep, non_cvd_ep, hfd_ep, acm_tte, ach_emerg_ep, ach_emerg_tte,
    matches("6mo")
  ) |>
  mutate(
...
file.remove(list.files(here::here("figs", "outcome_trends", "revision_hosp50"), full.names = TRUE))
...
for (t in seq_along(combined_list)) {
  export_table(
    combined_list[[t]], "outcome_trends/revision_hosp50",
    paste0(names(combined_list)[t], "_revision50")
  )
}
